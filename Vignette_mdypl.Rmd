...
```{r}
# standardized train set plus intercept ones
X_tr_stdi <- cbind(ones,X_tr_std)
```
Definition of the log-likelihood for the Diaconis-Ylvisaker penalized logistic regression
```{r}
loglikl <- function(beta,y,X){
  eta <- X%*%beta
  out <- sum(y*(eta)-log(1+exp(eta)))
  return(out)
}
```
`logistic_mDYPL()` computes the supremum of the log-likelihood as a function of $\boldsymbol{\beta}$
```{r}
logistic_mDYPL <- function(y_star,X,alpha,beta_init){
  
  nll <- function(beta){
    out <- -loglikl(beta,y_star,X) 
    return(out)
  }
  
  res <- suppressWarnings(nlm(f = nll, p = beta_init))
...
y_tr_star <- alpha*y_tr+(1-alpha)/2
beta_init <- rep(0,dim(X_tr_stdi)[2])

beta_init_fou <- rep(0,dim(X_tr_stdi[,1:77])[2])
```

Computation of the supremum of the log-likelihood for the full and nested models
```{r}
supll_full <- logistic_mDYPL(y_tr_star,X_tr_stdi,alpha,beta_init)
supll_fou<- logistic_mDYPL(y_tr_star,X_tr_stdi[,1:77],alpha,beta_init_fou)
```
...
```{r}
mod_full <- glm(y_tr ~ X_tr_std,family=binomial(),method="mdyplFit")
summ_full <- summary(mod_full,hd_correction=TRUE,corrupted=TRUE)

se_pars <- summ_full$se_parameters
...
int_ss2 <- expand.grid(intercept,ss2)
times <- dim(int_ss2)[1]
...
beta_init_fk <- rep(0,141)
beta_init_f <- rep(0,77)
const <- (1-alpha)/2
...
#   y_adj <- alpha*y+const
# 
#   supll_full <- logistic_mDYPL(y_adj,X_fki,alpha,beta_init_fk)
#   supll_fou <- logistic_mDYPL(y_adj,X_fki[,1:77],alpha,beta_init_f)
#   
#   PLR[t,i] <- 2*(supll_full-supll_fou)
